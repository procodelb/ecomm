...
export default function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    fetch("/api/auth/user").then((r) => {
      if (!r.ok) { window.location.href = "/login?redirect=" + encodeURIComponent(pathname); return; }
      setChecking(false);
    }).catch(() => { window.location.href = "/login?redirect=" + encodeURIComponent(pathname); });
  }, [pathname, router]);

  if (checking) {
    return <div className="flex min-h-screen items-center justify-center bg-dark"><div className="h-8 w-8 animate-spin rounded-full border-2 border-primary border-t-transparent" /></div>;
  }
